...
class ResUnet(torch.nn.Module):
    ...
    def get_encoder_features(self, img_a, img_b):
        ...
        Returns:
            ...
            encoder feature maps from each stage
        """
        # Extract features from source image (img_a) separately
        # Create dummy input with same shape as img_b but with zeros
        ...
        conv_num = 1
        while conv_num <= self.ndown_blocks:
            x_a, skip = getattr(self, 'down_{}'.format(conv_num))(x_a)
            features_a.append(skip)
            conv_num += 1
        ...
        conv_num = 1
        while conv_num <= self.ndown_blocks:
            x_b, skip = getattr(self, 'down_{}'.format(conv_num))(x_b)
            features_b.append(skip)
            conv_num += 1
