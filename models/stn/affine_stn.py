...
class AffineSTN(nn.Module):
    ...
    def _get_theta(self, img_a, img_b):
        ...
        theta = dtheta + self.identity_theta.to(dtheta.device).unsqueeze(0).repeat(bsize, 1)
        return theta
    ...
    def forward(self, img_a, img_b, apply_on=None):
        ...
        dtheta = self.net(img_a, img_b)
        theta = dtheta + self.identity_theta.to(dtheta.device).unsqueeze(0).repeat(img_a.size(0), 1)
