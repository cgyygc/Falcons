        ...
        for name in self.loss_names:
            if isinstance(name, str):
                ...
                if torch.is_tensor(loss):
                    # Use .mean() for non-scalar tensors, then .item() for Python scalar
                    if loss.dim() > 0:
                        errors_ret[name] = loss.mean().item()
                    else:
                        errors_ret[name] = loss.item()
                else:
                    errors_ret[name] = float(loss)  # float number
        return errors_ret
