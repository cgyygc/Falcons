"""
Registration Evaluation Script with Jacobian Folding Rate
...
- Jacobian Folding Rate (Percentage of voxels with negative Jacobian determinant)

Usage:
    python eval_registration_jacobian.py --dataroot ./datasets/rire --name rire2d_ukan_gbcm_contrastive_stable
"""
...
def ncc_metric(pred, target):
    ...
    Measures linear correlation between two images.
    """
...
def compute_jacobian_folding(disp_field):
    ...
    Args:
        disp_field: Deformation/displacement field tensor [B, 2, H, W] where 2=dy,dx
    ...
    def evaluate_batch(self, model, data):
        ...
        try:
            ...
            if hasattr(stn_wrapper, 'stn'):
                # ContrastiveSTNWrapper wraps the actual STN
                stn = stn_wrapper.stn
            else:
                stn = stn_wrapper

            # Try to get deformation field using offset_map
            if hasattr(stn, 'offset_map'):
                deformation_field = stn.offset_map(real_A, real_B)
                # offset_map returns displacement field [B, 2, H, W] or full grid [B, 4, H, W]
                if deformation_field.size(1) == 2:
                    disp_field = deformation_field  # [B, 2, H, W] - displacement field
                elif deformation_field.size(1) == 4:
                    # Full grid [x, y, 1-x, 1-y], extract displacement
            ...
            if disp_field is not None and disp_field.dim() == 4:
                folding_rate = compute_jacobian_folding(disp_field)
                self.jacobian_folding_rates.append(folding_rate)
            elif disp_field is None:
                print("Warning: Could not get deformation field - no offset_map method found")
            else:
                print(f"Warning: Deformation field has unexpected shape: {disp_field.shape}")
...
def main():
    ...
    results_file = os.path.join(results_dir, 'metrics_jacobian.txt')
