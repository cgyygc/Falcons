"""This module implements the RIRE 2D dataset class for loading pre-converted slice images.
...
The dataset structure should be:
    dataroot/
        ...
        or
        ct/              -- CT slice images
        mr_t1/           -- MR T1 slice images
        mr_t2/           -- MR T2 slice images
"""
import os
import random
...
class RIRE2DDataset(BaseDataset):
    """A dataset class for loading RIRE 2D slice images.

    This dataset loads pre-converted 2D PNG images from the RIRE dataset.
    ...
    def __init__(self, opt):
        ...
        print(f"RIRE 2D dataset: {len(self.matched_pairs)} image pairs loaded")
    ...
    def _match_pairs(self):
        ...
        for b_path in self.B_paths:
            ...
            # Format: patient_001_ct_slice_0050.png
            parts = b_filename.replace(f'{self.modality_b}_', '').split('_slice_')
            if len(parts) == 2:
                patient = parts[0]
        ...
        for a_path in self.A_paths:
            ...
            parts = a_filename.replace(f'{self.modality_a}_', '').split('_slice_')
            if len(parts) == 2:
                patient = parts[0]
                slice_id = parts[1].replace('.png', '')
                key = f"{patient}_slice_{slice_id}"
